...
# Method: Waypoint-based navigation with obstacle detection (Front_Sonar)
...
class AirSimWaypointNavigatorWithObstacleDetection:
    ...
    def detect_obstacle(self, max_distance=5.0):
        # Correct call: first argument is sensor name
        front_sonar = self.client.getDistanceSensorData("Front_Sonar")
    ...
    def drive_to_waypoint(self, target_x, target_y, tolerance=1.5, timeout=30):
        ...
        while time.time() - start_time < timeout:
            ...
            print(f"[{timestamp:.1f}] To ({target_x},{target_y}) pos=({pos.x_val:.1f},{pos.y_val:.1f}) err={angle_error:.2f}")
            self.csv_writer.writerow([timestamp, pos.x_val, pos.y_val, pos.z_val])

            if math.hypot(target_x - pos.x_val, target_y - pos.y_val) < tolerance:
